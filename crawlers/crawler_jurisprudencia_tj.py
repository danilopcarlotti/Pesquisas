...
class crawler_jurisprudencia_tj(crawlerJus):
    ...
    def capture_image_ESAJ(self, driver):
        ...
        try:
            captcha_image = re.search(
                r"url\(&quot\;(.*?)&quot", source, re.I | re.DOTALL
            ).group(1)
    ...
    def download_diarios_retroativos(self, link_diarios, datas_id, download_id, datas):
        ...
        for data in datas:
            driver.execute_script(
                'document.getElementById("%s").removeAttribute("readonly")' % datas_id
            )
            driver.find_element_by_id(datas_id).send_keys(data)
            driver.find_element_by_id(download_id).click()
            time.sleep(1)

    def download_pdf_acordao_captcha_audio(
        self, link, input_captcha_xpath, ouvir_captch_xpath, send_captcha, id_acordao
    ):
        ...
        driver = webdriver.Firefox(
            ...
            executable_path=common.download_path.path + "/geckodriver",
        )
        driver.get(link)
        try:
        ...
        while captcha_on:
            command = (
                'pacat --record -d %s | sox -t raw -r 44100 -s -L -b 16 -c 2 - "audio.wav" trim 0 6'
                % audio_monitor
            )
            ...
            driver.find_element_by_xpath(input_captcha_xpath).send_keys(
                self.captcha_audio(path="audio.wav")
            )
    ...
    def download_pdf_acordao_captcha_image(
        self, dados_baixar, input_captcha_xpath, send_captcha, prefix
    ):
        driver = webdriver.Chrome(self.chromedriver)
        ...
        for id_acordao, link in dados_baixar:
            ...
            time.sleep(1)
            subprocess.Popen(
                "mv %s/%s_*.pdf %s/%s" % (path, prefix, path, prefix), shell=True
            )
        driver.close()
    ...
    def download_tj_ESAJ(self, superC, data_julg_ini, data_julg_fim, termo="acordam"):
        ...
        superC.download_jurisprudencia(
            self,
            driver,
            self.pesquisa_livre,
            self.data_julgamento_inicialXP,
            data_julg_ini,
            self.data_julgamento_finalXP,
            data_julg_fim,
    ...
    def download_tj_ESAJ_recaptcha(
        self, superC, data_julg_ini, data_julg_fim, termo="acordam"
    ):
        cursor = cursorConexao()
        driver = webdriver.Chrome(self.chromedriver)
        driver.get(self.link_inicial)
