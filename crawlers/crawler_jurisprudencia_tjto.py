...
class crawler_jurisprudencia_tjto:
    ...
    def __init__(self):
        ...
        self.link_inicial = (
            "http://jurisprudencia.tjto.jus.br/consulta?q=a&start=%s&rows=20"
        )
    ...
    def download_acordao_to(self, id_acordao, link):
        crawler_jurisprudencia_tj.download_pdf_acordao_sem_captcha(
            self, link, "to_2_inst_" + id_acordao
        )
    ...
    def parser_acordaos(self):
        ...
        for arquivo in os.listdir(path + "/to_2_inst"):
            texto = (
                p.convert_pdfminer(path + "/to_2_inst/" + arquivo)
                .replace("\\", "")
                .replace("/", "")
                .replace('"', "")
            )
            ...
            cursor.execute(
                ...
                % ("rj", numero, data_decisao, orgao_julgador, julgador, texto)
            )
