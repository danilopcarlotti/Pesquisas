...
class crawler_proposicoes_legislativas:
    def __init__(self):
        ...
        self.link_inicial = "https://www.camara.leg.br/buscaProposicoesWeb/resultadoPesquisa?data={}&page=true&emtramitacao=Sim&regime=Urg%C3%AAncia".format(
            self.data
        )
    ...
    def download_csv(self):
        ...
        driver.find_element_by_xpath(
            '//*[@id="content"]/div[1]/form/ul[1]/li[4]/div/input'
        ).click()
        time.sleep(1)
    ...
    def upload_proposicoes(self):
        ...
        for index, row in df.iterrows():
            cursor.execute(
                'INSERT INTO governo_federal.proposicoes_legislativas_urg (proposicoes, ementa, explicacao, autor, uf, partido, apresentacao, situacao, link) values ("%s","%s","%s","%s","%s","%s","%s","%s","%s");'
                % (
                    str(row["Proposições"]).replace('"', ""),
                    str(row["Ementa"]).replace('"', ""),
                    str(row["Explicação "]).replace('"', ""),
                    str(row["Autor"]).replace('"', ""),
                    str(row["UF"]).replace('"', "")[:45],
                    str(row["Partido"])[:45].replace('"', ""),
                    str(row["Apresentaç"]).replace('"', "")[:45],
                    str(row["Situação"]).replace('"', ""),
                    str(row["Link "]).replace('"', ""),
                )
            )
