...
def insere_publicacoes_condicional(path_arquivo, collection_andamentos, dicionario_processos):
    print(path_arquivo)
    df = pd.read_csv(path_arquivo, chunksize=100)
    for chunk in df:
        for _, row in chunk.iterrows():
...
def main(path_diarios, uri_mongo, collection_name=None, condicional=False, dicionario_processos=None):
    ...
    for diario in diarios_processar:
        try:
            if condicional:
                insere_publicacoes_condicional(diario, collection_andamentos, dicionario_processos)
            else:
