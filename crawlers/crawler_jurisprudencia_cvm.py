...
class crawler_jurisprudencia_cvm():
	...
	def __init__(self):
		...
		self.file_links = open('/media/danilo/Seagate Expansion Drive/Links_cvm/lista_links.txt','a')
	...
	def download_cvm(self, ultima_pagina_visitada=1, ultima_pagina=197):
		...
		driver.find_element_by_xpath(self.numero_pagina_xp).send_keys(str(ultima_pagina_visitada))
		...
		while ultima_pagina:
			try:
				...
				driver.execute_script('arguments[0].scrollIntoView(true);', target)
				driver.find_element_by_xpath(self.numero_pagina_xp).clear()
				driver.find_element_by_xpath(self.numero_pagina_xp).send_keys(str(contador_pag))
				target = driver.find_element_by_xpath(self.botao_proximo_xp)
				driver.execute_script('arguments[0].scrollIntoView(true);', target)
