...
class crawler_jurisprudencia_stj(crawlerJus):
    ...
    def __init__(self, contador=1):
        ...
        self.lista_span_dados = [
            '//*[@id="idProcessoDetalhesBloco1"]/div[1]/span[2]',
    ...
    def baixarDadosProcesso(self, termo="a"):
        ...
        time.sleep(6)
        driver.find_elements_by_xpath('//*[@id="itemlistaresultados"]/span[2]/a')[
            2
        ].click()
        time.sleep(6)
        ...
        while self.contador < 62000:  # jun 2018
            ...
            try:
                acompanhamentos = driver.find_elements_by_xpath(
                    '//*[@id="acoesdocumento"]/a[2]'
                )
                for a in acompanhamentos:
                    ...
                    if len(info) == 10:
                        ...
                        link_voto = re.findall(
                            r"/processo/revista/documento/mediado/\?componente=ATC.*?tipo=\d+",
                            html,
                        )
                        if len(link_voto) > 0:
                            votos = ""
                            for decisao in link_voto:
                                link_voto_download = (
                                    "https://ww2.stj.jus.br" + decisao + "&formato=html"
                                )
                                link_voto_download = link_voto_download.replace(
                                    "amp;", ""
                                )
                                votos += link_voto_download + ","
                            info.append(votos[:-1])
                            cursor.execute(
                                'INSERT INTO stj.dados_processo_gisela (processo, recorrente, recorrido, autuacao, numero_unico, relator, ramo_direito, assunto, tribunal_origem, numeros_origem,link_voto) values ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")',
                                (
                                    info[0],
                                    info[1],
                                    info[2],
    ...
    def baixarVotos(self):
        ...
        for id_voto, link in link_votos:
            ...
            html_final = crawler.baixa_pag(
                "https://ww2.stj.jus.br/" + link_final["href"]
            )
            soup2 = BeautifulSoup(html_final, "html.parser")
            texto = soup2.get_text()
            cursor.execute(
                'INSERT into stj.votos (id_processo,voto) values (%s,"%s")'
                % (str(id_voto), texto.replace('"', ""))
            )
            print(contador)
            contador += 1
    ...
    def processar_arquivos_diarios(
        self, path_arquivos, path_csv_final="decisões_stj.csv"
    ):
        r = recursive_folders()
        rows = []
