...
class crawler_jurisprudencia_tjba:
    ...
    def __init__(self):
        ...
        self.botao_proximoXP = (
            '//*[@id="jurisprudenciaGridId_paginator_top"]/span[5]/span'
        )
    ...
    def download_tj(self, termo="acordam"):
        ...
        while True:
            try:
                ...
                subprocess.Popen(
                    "mv %s/ba_2_inst_*.pdf %s/ba_2_inst" % (path, path), shell=True
                )
                driver.switch_to.window(driver.window_handles[0])
                driver.execute_script(
                    "return arguments[0].scrollIntoView();",
            ...
            if re.search(r"/unicorp/images/.*?pdf", l["href"]):
                try:
                    urllib.request.urlretrieve(
                        "http://www5.tjba.jus.br" + l["href"],
                        "TJBA_1_inst_%s.pdf" % str(contador),
                    )
                    subprocess.Popen(
                        "mv TJBA_1_inst_*.pdf %s/ba_1_inst" % (path,), shell=True
                    )
                    contador += 1
                except Exception as e:
                    print(e)
    ...
    def parser_acordaos(self, arquivo, cursor, pdf_class):
        ...
        orgao_julgador = busca(
            r"\s*?PODER JUDICIÁRIO DO ESTADO DA BAHIA\nTRIBUNAL DE JUSTIÇA\n(.*?)\n",
            texto,
        )
        try:
            cursor.execute(
                'INSERT INTO jurisprudencia_2_inst.jurisprudencia_2_inst (tribunal, numero, assunto, data_decisao, orgao_julgador, julgador, texto_decisao) values ("%s","%s","%s","%s","%s","%s","%s");'
                % ("ba", numero, assunto, data_decisao, orgao_julgador, julgador, texto)
            )
        except:
            pass
