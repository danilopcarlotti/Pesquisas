...
class crawler_jurisprudencia_tjsc(crawler_jurisprudencia_tj):
            ...
            for e in cadernos:
                try:
                    ...
                    response = urllib.request.urlopen(
                        link_inicial % (str(i), e), timeout=15
                    )
    ...
    def download_tj(self, data_ini, data_fim, termo="processo"):
        ...
        driver.find_element_by_xpath(
            '//*[@id="busca_avancada"]/table[1]/tbody/tr/td[1]/span[1]'
        ).click()
    ...
    def download_acordao_sc(self, link, id_p, cursor):
        ...
        texto = (
            crawlerclass.baixa_texto_html(link)
            .replace('"', "")
            .replace("\\", "")
            .replace("/", "")
        )
    ...
    def parser_acordaos(self, texto, cursor):
        ...
        origem = busca(r"\n\s*?Origem\s*?\:(.*?)\n", texto)
        cursor.execute(
            'INSERT INTO jurisprudencia_2_inst.jurisprudencia_2_inst_sc (tribunal, numero, classe, data_decisao, orgao_julgador, julgador, origem, texto_decisao) values ("%s","%s","%s","%s","%s","%s","%s","%s");'
            % (
                "sc",
                numero,
                classe,
