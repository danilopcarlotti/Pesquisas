...
class publicacoes_diarios_oficiais(crawlerJus):
    def __init__(self, data=None):
        ...
        logging.basicConfig(
            filename=self.cwd + self.dia + self.mes + self.ano + ".log",
            level=logging.INFO,
        )
    ...
    def baixaEsaj(self, inicio, fim, pag):
        ...
        for i in range(inicio, fim):
            ...
            driver.find_element_by_xpath(
                XPathInicial + str(int(i)) + XPathFinal
            ).click()
        ...
        if todos:
            ...
            while aux:
                ...
                for l in link_ac_i:
                    ...
                    subprocess.Popen(
                        'mv %s/%s.pdf "%s/Diarios_ac/dir_005/%s.pdf"'
                        % (
                            os.getcwd(),
                            "TJAC_" + str(counter),
                            path_diarios,
            ...
            subprocess.Popen(
                ...
                % (
                    path,
                    path_diarios,
                    "TJAC_" + str(self.dia) + str(self.mes) + str(self.ano),
                ),
                shell=True,
            )
    ...
    def baixa_al(self):
        if self.data:
            ...
            c.download_diario_retroativo(
                data_especifica=self.dia_pesquisar
                + "/"
                + self.mes_pesquisar
                + "/"
                + self.ano_pesquisar
            ...
            subprocess.Popen(
                "mv %s/*.pdf %s/TJAL_%s.pdf"
                % (path, path_diarios, self.dia + self.mes + self.ano),
                shell=True,
            )
    ...
    def baixa_ap(self, ultimo_download=2765):
        ...
        driver.execute_script(
            "document.getElementById('dje-%s').click()" % (str(ultimo_download + 1),)
        )
        print(
            driver.execute_script(
                "document.getElementById('dje-%s')" % (str(ultimo_download + 1),)
            )
        )
    ...
    def baixa_df(self):
        ...
        link_df_i = pag_df_bs.find(
            "a",
            href=re.compile(
                "https://dje.tjdft.jus.br/dje/jsp/dje/DownloadDeDiario.jsp"
            ),
        )
        ...
        time.sleep(15)
        subprocess.Popen(
            "mv %s/*.PDF %s/TJDF_%s.pdf"
            % (path, path_diarios, str(self.dia) + str(self.mes) + str(self.ano)),
            shell=True,
        )

    def baixa_es(self):
        diario_es = open(
            self.dia + self.mes + self.ano + "es.txt", "a", encoding="utf-8"
        )
        ...
        for l in link_es_f:
            try:
                ...
                texto_es_f = re.search(
                    r"Versão revista(.*?)O e-diário \(Diário da Justiça Eletrônico",
                    texto_es_i,
                    re.DOTALL,
                )
                if texto_es_f != None:
                    ...
                    texto_es_ff = re.sub(
                        r"O e-diário \(Diário da Justiça Eletrônico", "", texto_es_ff
                    )
        ...
        subprocess.Popen(
            "mv %s/%s*.pdf %s"
            % (os.getcwd(), self.dia + self.mes + self.ano + "es.txt", path_diarios),
            shell=True,
        )
    ...
    def baixa_go(self):
        ...
        for pasta in pastas:
            ...
            for link in links_diarios_GO:
                for j in range(1, 4):
                    pag_diarios_mes = self.baixa_pag(
                        "http://tjdocs.tjgo.jus.br%s?page=%s" % (link, str(j))
                    )
                    pag_diarios_mes_bs = BeautifulSoup(pag_diarios_mes, "html.parser")
                    links_diarios = pag_diarios_mes_bs.find_all(
                        "a", href=re.compile(r"/documentos/\d+")
                    )
                    for link_d in links_diarios:
                        link_final = (
                            "http://tjdocs.tjgo.jus.br" + link_d["href"] + "/download"
                        )
                        driver = webdriver.Chrome(self.chromedriver)
                        ...
                        driver.close()
                        subprocess.Popen(
                            'mv %s/*.pdf "%s/Diarios_go/"' % (path, path_diarios),
                            shell=True,
                        )
    ...
    def baixa_mt(self):
        ...
        for l in range(1, len(link_mt_f)):
            ...
            time.sleep(5)
            subprocess.Popen(
                "mv %s/*.pdf %s/%s.pdf" % (os.getcwd(), path_diarios, filename),
                shell=True,
            )
    ...
    def baixa_pa(self):
        ...
        driver.find_element_by_css_selector(
            "a[ng-click='abrirPDFGrid(urlUltimoDiario)']"
        ).click()
        time.sleep(7)
        driver.close()
    ...
    def baixa_pi(self):
        ...
        link_pi_i = pag_pi_bs.find(
            "a", href=re.compile(r"http://www\.tjpi\.jus\.br/diarioeletronico/public.+")
        )
    ...
    def baixa_pr(self):
        ...
        if self.data:
            driver.find_element_by_name("dataVeiculacao").send_keys(
                self.dia_pesquisar + "/" + self.mes_pesquisar + "/" + self.ano_pesquisar
            )
        else:
            driver.find_element_by_name("dataVeiculacao").send_keys(
    ...
    def baixa_rn(self):
        ...
        driver.get(rn_dje)
        driver.find_element_by_xpath(
            "/html/body/table/tbody/tr/td/table/tbody/tr[4]/td/table[2]/tbody/tr/td/div/a"
        ).click()
        driver.find_element_by_xpath('//*[@id="menu:formMenu:_id16"]').click()
        driver.find_element_by_xpath(
            '//*[@id="pesquisarEdicaoCompletaBean:pesquisa_:_id45"]'
        ).click()
        time.sleep(3)
        link_rn_f = (
            "https://www.diario.tjrn.jus.br/djonline/pages/repositoriopdfs/"
            + ano
            + "/"
            + tri
            + "/"
    ...
    def baixa_ro(self):
        response = urllib.request.urlopen(
            "https://portal.tjro.jus.br/diario-api/ultimo-diario.php", timeout=5
        )
    ...
    def baixa_rr(self):
        if self.data:
            link_final_rr = (
                "http://diario.tjrr.jus.br/dpj/dpj-"
                + self.ano_pesquisar
                + self.mes_pesquisar
                + self.dia_pesquisar
                + ".pdf"
            ...
            file5 = open(
                self.dia_pesquisar + self.mes_pesquisar + self.ano_pesquisar + "RR.pdf",
                "wb",
            )
        else:
            link_final_rr = (
    ...
    def baixa_rs(self):
        ...
        for i in lista_rs:
            driver.get(
                "http://www3.tjrs.jus.br/servicos/diario_justica/download_edicao.php?tp="
                + i
                + "&ed="
                + n_edicao_rs_f
            )
            ...
            subprocess.Popen(
                ...
                % (
                    path,
                    path_diarios,
                    i + "_TJRS_" + str(self.dia) + str(self.mes) + str(self.ano),
                ),
                shell=True,
            )
    ...
    def baixa_sc(self):
        ...
        time.sleep(1)
        driver.find_element_by_xpath(
            '//*[@id="div_diarios_publicados"]/span[2]/span/div/ul/li/span/a'
        ).click()
        time.sleep(5)
        driver.switch_to.window(driver.window_handles[-1])
    ...
    def baixa_se(self):
        ...
        link_se_f = (
            "http://www.diario.tjse.jus.br/diario/diarios/"
            + link_se_i[-1].text.split("(")[0]
            + ".pdf"
        )
        ...
        file.close()
        subprocess.Popen(
            "mv %s/*.pdf %s/TJSE_%s.pdf"
            % (os.getcwd(), path_diarios, self.dia + self.mes + self.ano),
            shell=True,
        )
    ...
    def baixa_stf(self, ultimo_download=191):
        if self.data:
            ...
            link_final_stf = (
                "https://www.stf.jus.br/arquivo/djEletronico/DJE_%s_%s.pdf"
                % (
                    self.ano_pesquisar + self.mes_pesquisar + self.dia_pesquisar,
                    ultimo_download,
                )
            )
            ...
            link_final_stf = (
                "https://www.stf.jus.br/arquivo/djEletronico/DJE_%s_%s.pdf"
                % (self.ano + self.mes + self.dia, n_diario_stf_f[0])
            )
    ...
    def baixa_to(self):
        ...
        link_to_i = pag_to_bs.find(
            "a", href=re.compile("http://wwa\.tjto\.jus\.br/diario/diariopublicado/.+")
        )
    ...
    def baixa_trf1(self):
        ...
        links_trf1_i = pag_trf1_ini_bs.find_all(
            "a", href=re.compile(r"/edj/handle/123/.+")
        )
        links_trf1_i2 = []
        links_trf1_f = []
        ...
        for ltrf1_2 in links_trf1_i2:
            ...
            links_trf1_aux = aux_link_trf1_bs.find(
                "a", href=re.compile(r"/edj/bitstream/handle/123.+")
            )
    ...
    def baixa_trf2(self):
        ...
        driver.get(trf2_dje)
        driver.find_element_by_xpath(
            '//*[@id="ctl00_ContentPlaceHolder_ctrInicial_ctrCadernosPorAreaJudicial_lkbCadJudTRF"]'
        ).click()
        driver.find_element_by_xpath(
            '//*[@id="ctl00_ContentPlaceHolder_ctrInicial_ctrCadernosPorAreaJudicial_lkbCadJudSJRJ"]'
        ).click()
        driver.find_element_by_xpath(
            '//*[@id="ctl00_ContentPlaceHolder_ctrInicial_ctrCadernosPorAreaJudicial_lkbCadJudSJES"]'
        ).click()
        time.sleep(15)
    ...
    def baixa_trf3(self):
        ...
        for i in range(2, 3):
            ...
            driver.find_element_by_xpath(
                '//*[@id="ultimaEdicao"]/li[' + str(i) + "]/a"
            ).click()
            time.sleep(17)
            subprocess.Popen(
                "mv %s/*.pdf %s/TRF3_%s.pdf"
                % (path, path_diarios, str(i) + self.ano + self.mes + self.dia),
                shell=True,
            )
        for i in range(3, 10):
            driver.find_element_by_xpath('//*[@id="botao-ultima"]/a').click()
    ...
    def baixa_trf4(self):
        ...
        try:
            ...
            file.close()
            subprocess.Popen(
                "mv %s/*.pdf %s/TRF4_%s" % (os.getcwd(), path_diarios, dia + mes + ano),
                shell=True,
            )
    ...
    def baixa_trf5(self):
        ...
        def trf5_baixa_diarios(orgao):
            ...
            time.sleep(1)
            driver.execute_script(
                "return oamSubmitForm('frmPesquisa','frmPesquisa:tDiarios:0:j_id67','_blank',[]);"
            )
            time.sleep(3)
            driver.close()
    ...
    def baixa_trt(self):
        ...
        subprocess.Popen(
            ...
            % (
                os.getcwd(),
                path_diarios,
                "Diario_TST_%s.pdf" % (self.dia + self.mes + self.ano,),
            ),
            shell=True,
        )
        for l in range(1, len(links_trt)):
            ...
            file.close()
            subprocess.Popen(
                "mv %s/*.pdf %s/%s" % (os.getcwd(), path_diarios, filename), shell=True
            )
...
if __name__ == "__main__":
    ...
    for data_p in datas_p[3:]:
        ...
        for i in range(2, 4):
            ...
            subprocess.Popen(
                'mkdir "%s/Diarios_ms/%s"' % (path_diarios, nome_pasta), shell=True
            )
            subprocess.Popen(
                'mv %s/*.pdf "%s/Diarios_ms/%s/"' % (path, path_diarios, nome_pasta),
                shell=True,
            )
        driver.quit()
