...
class crawler_jurisprudencia_tjms:
    ...
    def parser_acordaos(self, texto, cursor):
        def parse(texto_decisao, cursor):
            numero = busca(
                r"\d{7}\-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}", texto_decisao, ngroup=0
            )
            ...
            cursor.execute(
                'INSERT INTO jurisprudencia_2_inst.jurisprudencia_2_inst (tribunal, numero, assunto, classe, data_decisao, orgao_julgador, julgador, texto_decisao, origem) values ("%s","%s","%s","%s","%s","%s","%s","%s","%s");'
                % (
                    "ms",
                    numero,
                    assunto,
