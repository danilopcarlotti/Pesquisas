...
def alagoas(termo=None):
    ...
    for l in c.lista_anos:
        try:
            print(l, "\n")
            if termo:
                crawler_jurisprudencia_tj.download_tj_ESAJ_recaptcha(
                    c,
                    crawler_jurisprudencia_tj,
                    "01/01/" + l,
                    "31/12/" + l,
                    termo=termo,
                )
                ...
                crawler_jurisprudencia_tj.download_tj_ESAJ_recaptcha(
                    c, crawler_jurisprudencia_tj, "01/01/" + l, "31/12/" + l
                )
        except Exception as e:
            print(e)
...
def amazonas(termo=None):
    ...
    try:
        for l in range(3, len(c.lista_anos)):
            ...
            for m in range(len(c.lista_meses)):
                try:
                    if termo:
                        if m == 1:
                            crawler_jurisprudencia_tj.download_tj_ESAJ(
                                c,
                                crawler_jurisprudencia_tj,
                                "01" + c.lista_meses[m] + c.lista_anos[l],
                                "28" + c.lista_meses[m] + c.lista_anos[l],
                                termo=termo,
                            )
                            ...
                            crawler_jurisprudencia_tj.download_tj_ESAJ(
                                ...
                                crawler_jurisprudencia_tj,
                                "01" + c.lista_meses[m] + c.lista_anos[l],
                                "31" + c.lista_meses[m] + c.lista_anos[l],
                                termo=termo,
                            )
                        else:
                            crawler_jurisprudencia_tj.download_tj_ESAJ(
                                ...
                                crawler_jurisprudencia_tj,
                                "01" + c.lista_meses[m] + c.lista_anos[l],
                                "30" + c.lista_meses[m] + c.lista_anos[l],
                                termo=termo,
                            )
                    else:
                        if m == 1:
        ...
        print(e)
    cursor.execute(
        "SELECT id,ementas from justica_estadual.jurisprudencia_am where id > 29237 limit 10000000;"
    )
...
def distrito_federal(termo=None):
    ...
    try:
        ...
        print(e)
    subprocess.Popen(
        "cd %s;for A in *.doc; do libreoffice --headless --convert-to docx $A; done; for A in *.doc; do rm $A; done;"
        % (path + "/df_2_inst/",),
        shell=True,
    )
    cursor = cursorConexao()
        ...
        for a in c.lista_anos:
            for m in range(len(c.lista_meses)):
                if termo:
                    if m == 1:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a,
                            "28" + c.lista_meses[m] + a,
                            termo=termo,
                        )
                    elif m in [0, 2, 4, 6, 7, 9, 11]:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a,
                            "31" + c.lista_meses[m] + a,
                            termo=termo,
                        )
                    else:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a,
                            "30" + c.lista_meses[m] + a,
                            termo=termo,
                        )
                else:
                    if m == 1:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a, "28" + c.lista_meses[m] + a
                        )
                    elif m in [0, 2, 4, 6, 7, 9, 11]:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a, "31" + c.lista_meses[m] + a
                        )
                    else:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a, "30" + c.lista_meses[m] + a
                        )
    except Exception as e:
        print(e)
            ...
            for m in range(len(c.lista_meses)):
                if termo:
                    if m == 1:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a,
                            "14" + c.lista_meses[m] + a,
                            termo=termo,
                        )
                        c.download_tj(
                            "15" + c.lista_meses[m] + a,
                            "28" + c.lista_meses[m] + a,
                            termo=termo,
                        )
                    elif m in [0, 2, 4, 6, 7, 9, 11]:
                    ...
                    if m == 1:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a, "14" + c.lista_meses[m] + a
                        )
                        c.download_tj(
                            "15" + c.lista_meses[m] + a, "28" + c.lista_meses[m] + a
                        )
                    elif m in [0, 2, 4, 6, 7, 9, 11]:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a, "14" + c.lista_meses[m] + a
                        ...
                        c.download_tj(
                            "15" + c.lista_meses[m] + a, "31" + c.lista_meses[m] + a
                        )
                    else:
                        c.download_tj(
                            "01" + c.lista_meses[m] + a, "14" + c.lista_meses[m] + a
                        ...
                        c.download_tj(
                            "15" + c.lista_meses[m] + a, "30" + c.lista_meses[m] + a
                        )
    except Exception as e:
        print("finalizei com erro ", e)
            ...
            if termo:
                for m in range(len(c.lista_meses)):
                    for i in range(1, 8):
                        try:
                            c.download_tj(
                                "0" + str(i) + "/" + c.lista_meses[m] + "/" + a,
                                "0" + str(i + 1) + "/" + c.lista_meses[m] + "/" + a,
                                termo=termo,
                            )
                        except Exception as e:
                            print(e)
                    for i in range(10, 27):
                        try:
                            c.download_tj(
                                str(i) + "/" + c.lista_meses[m] + "/" + a,
                                str(i + 1) + "/" + c.lista_meses[m] + "/" + a,
                                termo=termo,
                            )
                        except Exception as e:
                            print(e)
...
def paraiba(termo=None):
    ...
    try:
        ...
        print(e)
    cursor.execute(
        "SELECT id,ementas from justica_estadual.jurisprudencia_pb where id > 37630 limit 10000000;"
    )
    lista_links = cursor.fetchall()
...
def rio_grande_do_norte(termo=None):
    ...
    cursor = cursorConexao()
    cursor.execute(
        "SELECT ementas from justica_estadual.jurisprudencia_rn limit 1000000"
    )
    dados = cursor.fetchall()
    for dado in dados:
        c.parser_acordaos(dado[0], cursor)
            ...
            for m in range(len(c.lista_meses)):
                for i in range(1, 8):
                    try:
                        if termo:
                            c.download_tj(
                                "0" + str(i) + c.lista_meses[m] + c.lista_anos[l],
                                "0" + str(i + 1) + c.lista_meses[m] + c.lista_anos[l],
                                termo=termo,
                            )
                        else:
                            c.download_tj(
                ...
                for i in range(10, 27):
                    try:
                        if termo:
                            c.download_tj(
                                str(i) + c.lista_meses[m] + c.lista_anos[l],
                                str(i + 1) + c.lista_meses[m] + c.lista_anos[l],
                                termo=termo,
                            )
                        else:
                            c.download_tj(
...
def sao_paulo_1(termo=None):
    ...
    for i in range(0, 1500000, 10000):  # jun 2018
        print(1500000 - i)
        cursor.execute(
            "SELECT sentencas FROM justica_estadual.jurisprudencia_sp_1_inst limit %s,10000;"
            % str(i)
        )
        dados = cursor.fetchall()
        for dado in dados:
...
def supremo_tribunal_federal(termo=None):
    ...
    for id_l, link_pag in links:
        try:
            cursor.execute(
                'UPDATE stf.texto_decisoes set id_processo = ("%s") where id = ("%s")'
                % (links_pai_d[int(link_pag.split("incidente=")[1])], id_l)
            )
        except Exception as e:
            print(e)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script para download de jurisprudência dos tribunais disponíveis"
    )
    parser.add_argument(
        "-t",
        type=str,
        default=False,
        help="termo a ser utilizado como referencia na pesquisa",
        dest="t",
    )
