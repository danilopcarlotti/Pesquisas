...
class crawler_jurisprudencia_tjmg:
    ...
    def __init__(self):
        ...
        self.botao_proximo_XP = (
            '//*[@id="textoUmaColuna"]/table[1]/tbody/tr[4]/td/table/tbody/tr/td[5]/a'
        )
        self.link_download_captcha = (
            "/html/body/table/tbody/tr[3]/td/table/tbody/tr[4]/td/a[2]"
        )
    ...
    def download_tj(self, data_ini, data_fim, termo="direito"):
        ...
        time.sleep(5)
        driver.find_element_by_xpath(self.link_captcha).send_keys(
            crawler_jurisprudencia_tj.captcha(self)
        )
        crawler_jurisprudencia_tj.delete_audios(self)
        while True:
            try:
    ...
    def parser_acordaos(self, texto, cursor):
        ...
        cursor.execute(
            'INSERT INTO jurisprudencia_2_inst.jurisprudencia_2_inst (tribunal, numero, origem, data_decisao, orgao_julgador, julgador, texto_decisao) values ("%s","%s","%s","%s","%s","%s","%s");'
            % (
                "mg",
                numero,
                origem,
...
if __name__ == "__main__":
    ...
    cursor = cursorConexao()
    cursor.execute(
        "SELECT ementas from justica_estadual.jurisprudencia_mg limit 1000000"
    )
