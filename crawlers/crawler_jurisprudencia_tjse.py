...
class crawler_jurisprudencia_tjse(crawler_jurisprudencia_tj):
    ...
    def __init__(self):
        ...
        self.botao_proximoXP = (
            '//*[@id="dgResultadoJurisprudencia2_paginator_top"]/a[3]'
        )
    ...
    def parser_acordaos(self, texto, cursor):
        ...
        for d in range(1, len(decisoes)):
            ...
            cursor.execute(
                'INSERT INTO jurisprudencia_2_inst.jurisprudencia_2_inst (tribunal, numero, assunto, classe, data_decisao, orgao_julgador, julgador, polo_ativo, polo_passivo, texto_decisao) values ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s");'
                % (
                    "se",
                    numero,
                    assunto,
