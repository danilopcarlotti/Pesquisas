...
class crawler_jurisprudencia_tjpb(crawler_jurisprudencia_tj):
    ...
    def download_tj(self, dataIni, dataFim, termo="a"):
        ...
        for l in links_inteiro_teor:
            cursor.execute(
                ...
                % (self.tabela_colunas, l.get_attribute("href"))
            )
    ...
    def download_acordao_pb(self, id_acordao, link):
        crawler_jurisprudencia_tj.download_pdf_acordao(
            self, link, "", "", "", "pb_2_inst_" + id_acordao
        )
        subprocess.Popen(
            "mv /home/danilo/Downloads/pb_2_inst_* /home/danilo/Downloads/acordaos_tj_pb",
            shell=True,
        )
    ...
    def parser_acordaos(self, arquivo, cursor, pdf_class):
        ...
        cursor.execute(
            'INSERT INTO jurisprudencia_2_inst.jurisprudencia_2_inst (tribunal, numero, classe, data_decisao, julgador, texto_decisao) values ("%s","%s","%s","%s","%s","%s");'
            % (
                "pb",
                numero,
                classe,
