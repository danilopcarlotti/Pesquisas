...
def extrair_data_al(path_arquivo):
    ...
    if data:
        data_encontrada = data.group(1)
        return (
            data_encontrada[:2] + "/" + data_encontrada[2:4] + "/" + data_encontrada[4:]
        )
    return " "
...
def extrair_data_mt(arq):
    data_raw = re.search(
        r"(\d{1,2}) de (.*?) de (\d{4})", arq[:30000], flags=re.DOTALL | re.I
    )
    if data_raw:
        day = data_raw.group(1)
        if len(day) == 1:
...
def extrair_data_rr(path_arquivo):
    ...
    if data:
        data_encontrada = data.group(1)
        return (
            data_encontrada[6:] + "/" + data_encontrada[4:6] + "/" + data_encontrada[:4]
        )
    return " "
    ...
    if tribunal in dicionario_separacao_diarios:
        ...
        for pub in publicacoes:
            if pub:
                pub = "".join(pub)
                numero = (
                    encontra_numero(tribunal, pub).replace("\n", "").replace(" ", "")
                )
                rows.append(
                    {
                        "tribunal": tribunal,
                        "texto_publicacao": pub.replace("\n", " "),
    ...
    for path_arquivo in diarios_processar:
        ...
        if tribunal:
            ...
            if input_type == "path_arquivo":
                ...
                input_f = " ".join(
                    [line for line in open(path_arquivo.replace("csv", "txt"), "r")]
                )
            try:
