    ...
    def download_portais(self, download_path, final_path, ano_xpath):
        for _, row in self.df_portais.iterrows():
            ...
            if input("Clique em download\n"):
                ...
                subprocess.Popen(
                    'mv %s/*.csv "%s/%s/"' % (download_path, final_path, nome_pasta),
                    shell=True,
                )
            driver.close()
...
if __name__ == "__main__":
    c = crawler_portais_transparencia(
        "/mnt/Dados/Documents/pesquisas_privado_dados/compras_publicas/"
    )
    print(
        c.download_portais(
            "/home/danilo/Downloads",
            "/mnt/Dados/Documents/pesquisas_privado_dados/compras_publicas/Portais transparência/Transparencia 2019",
            '//*[@id="cmbExercicio_DDD_L_LBI6T0"]',
        )
    )
