...
def main(path_files, qualificador):
    ...
    df_descritivo_pub.to_csv(
        "descritivo_numero_publicações_ano{}.csv".format(qualificador), index=False
    )
    df_descritivo_diarios = pd.DataFrame(rows_descritivo_n_diario)
    df_descritivo_diarios.to_csv(
        "descritivo_numero_diarios_ano{}.csv".format(qualificador), index=False
    )
