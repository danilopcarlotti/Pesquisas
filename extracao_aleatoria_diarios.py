...
def pesquisa_diario(df_path, textos_interesse, expressions_true=REGULAR_EXPRESSIONS_TRUE,
    expressions_false=REGULAR_EXPRESSIONS_FALSE, column_text='texto_publicacao', column_id='numero_processo'):
    df = pd.read_csv(df_path, chunksize=100, usecols=[column_text, column_id], engine='python')
    for chunk in df:
...
def pesquisa_diario_to_csv(df_path, rows, expressions_true=REGULAR_EXPRESSIONS_TRUE,
    expressions_false=REGULAR_EXPRESSIONS_FALSE, column_text='texto_publicacao', column_id='numero_processo'):
    df = pd.read_csv(df_path, chunksize=100, usecols=[column_text, column_id, 'tribunal', 'data'], engine='python')
    for chunk in df:
        for _, row in chunk.iterrows():
            aux = True
            for expression in expressions_true:
...
if __name__ == "__main__":
    r = recursive_folders()
    paths = [i for i in r.find_files(sys.argv[1]) if i[-4:] == '.csv' and re.search(r'2020',i)]
    ...
    for diario in paths:
        ...
        if len(rows) > 10000:
            ...
            df_new.to_csv('{}/extracao_{}.csv'.format(str(sys.argv[2]),str(counter)),index=False)
            counter += 1
            rows = []
        gc.collect()
    if len(rows):
