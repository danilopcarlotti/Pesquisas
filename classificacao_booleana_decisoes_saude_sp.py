	...
	def variaveis_textos(self, dicionario, dados):
		...
		for id_p, texto, tribunal, data_decisao, polo_ativo, polo_passivo, origem in dados:
			try:
				...
				if ano_processo:
					ano_processo = ano_processo.group(1)
				else:
					ano_processo = ''
				dicionario_df = {'numero':id_p, 'ano': ano_processo, 'tribunal':tribunal, 'data_decisao':data_decisao,'polo_ativo':polo_ativo,'polo_passivo':polo_passivo,'origem':origem,'estadual_federal':est_ou_fed,'resultado':self.parser.indicaResultado(texto,'Sentença'),'justica_gratuita':self.parser.justica_gratuita(texto)}
...
def main():
	...
	df.to_csv('relatorio_booleano_cnj_08_nov_2018_sp.csv',quotechar='"', index= False)
