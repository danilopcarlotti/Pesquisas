...
class extracao_variaveis:
    ...
    def __init__(self):
        ...
        self.var_bool["Rename - remédio"] = "|".join(
            [i.replace(" + ", ".{1,20}").strip() for i in open("rename.txt", "r")]
        )
        self.var_bool["Medicamentos atenção básica"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open("medicamentos_atencao_basica.txt", "r")
            ]
        )
        self.var_bool["Medicamentos componente especializado"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open("medicamentos_componente_especializado.txt", "r")
            ]
        )
        self.var_bool["Medicamentos estratégicos"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open("medicamentos_estrategicos.txt", "r")
            ]
        )
    ...
    def variaveis_textos(self, dicionario, dados):
        ...
        index = []
        index_counter = 1
        for id_p, tribunal, numero, data, texto in dados:
            try:
                est_ou_fed = "estadual"
...
def main():
    ...
    df = ext.variaveis_textos(ext.var_bool, dados)
    df.to_csv(
        "relatorio_booleano_cnj_03_jan_2019_tutelas.csv", quotechar='"', index=False
    )
