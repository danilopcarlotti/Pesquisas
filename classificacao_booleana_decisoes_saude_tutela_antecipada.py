...
def main():
	...
	for chunk in df:
		for index, row in chunk.iterrows():
			if re.search(r'(concedo|autorizo|defiro).{1,30}(antecipa..o.{1,20}tutela|tutela.{1,20}emerg.ncia|tutela.{,20}antecipada)',row['texto_publicacao']) and not re.search(r'penal',row['texto_publicacao'],re.I):
				dados.append((row['id'],row['tribunal'],row['numero_processo'],row['data'],row['texto_publicacao']))
	df = ext.variaveis_textos(ext.var_bool, dados)
	df.to_csv('relatorio_booleano_cnj_03_jan_2019_tutelas.csv',quotechar='"', index= False)
