...
class classifier_texts_tfidf:
    ...
    def create_X_y(self):
        self.normal_texts = [
            " ".join(i) for i in self.txtN.normalize_texts(self.df["text"].tolist())
        ]
    ...
    def dump_clf(self, prefix="", titulo="", dump_vectorizer=True):
        if dump_vectorizer:
            pickle.dump(
                self.vectorizer, open(prefix + "vectorizer_%s.pickle" % (titulo,), "wb")
            )
            if self.use_pca:
                pickle.dump(
                    self.pca_vectorizer,
                    open(prefix + "pca_vectorizer_%s.pickle" % (titulo,), "wb"),
                )
        pickle.dump(
            self.classifier_parent,
            open(prefix + "classifier_parent_clf_%s.pickle" % (titulo,), "wb"),
        )
...
if __name__ == "__main__":
    ...
    # # LOGISTIC REGRESSION
    clf = classifier_texts_tfidf(
        LogisticRegression(penalty="l1", n_jobs=-1, solver="saga"),
        use_pca=False,
        df_path=sys.argv[1],
    )
