...
class vectorizer_texts_tfidf:
    ...
    def create_X_y(self, df_column_text, df_class_text):
        self.normal_texts = [
            " ".join(i)
            for i in self.txtN.normalize_texts(self.df[df_column_text].tolist())
        ]
    ...
    def dump_clf(self, prefix="", titulo=""):
        pickle.dump(
            self.vectorizer, open(prefix + "vectorizer%s.pickle" % (titulo,), "wb")
        )
        if self.use_pca:
            pickle.dump(
                self.pca_vectorizer,
                open(prefix + "pca_vectorizer%s.pickle" % (titulo,), "wb"),
            )
