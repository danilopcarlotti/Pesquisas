...
# This program needs the libraries available in https://github.com/danilopcarlotti/Pesquisas
...
def texts_of_interest(csv_path, indexes, labels, path_reports):
    ...
    for i in range(len(texts_of_interest)):
        doc = Document()
        doc.add_paragraph(
            "Text representative of cluster: " + str(labels[indexes[i]]) + "\n\n"
        )
        doc.add_paragraph(
            "Number of texts in the cluster: "
            + str(numero_textos_por_cluster[labels[indexes[i]]])
            + "\n\n"
        )
        doc.add_paragraph(texts_of_interest[i])
        doc.save(
            path_reports
            + "text_representative_of_cluster_%s.docx" % (str(labels[indexes[i]]),)
        )
