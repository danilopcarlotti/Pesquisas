...
class topicModelling(textNormalization):
	...
	def topic_to_txt(self, topics, nome_topicos='', prefix='', num_topics=5,num_words=15):
		for n,top in topics.print_topics(num_topics=num_topics,num_words=num_words):
			...
			subprocess.Popen('wordcloud_cli --text %s --imagefile "%swordcloud_%s.png" --no_collocations' % ('wordcloud_topico_'+str(n)+nome_topicos+'.txt',prefix,str(n)+nome_topicos),shell=True)
