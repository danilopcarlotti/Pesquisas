...
class topicModelling(textNormalization):
    ...
    def topic_to_txt(
        self, topics, nome_topicos="", prefix="", num_topics=5, num_words=15
    ):
        for n, top in topics.print_topics(num_topics=num_topics, num_words=num_words):
            ...
            subprocess.Popen(
                'wordcloud_cli --text %s --imagefile "%swordcloud_%s.png" --no_collocations'
                % (
                    "wordcloud_topico_" + str(n) + nome_topicos + ".txt",
                    prefix,
                    str(n) + nome_topicos,
                ),
                shell=True,
...
if __name__ == "__main__":
    dados_1_inst = (
        "/home/ubuntu/topicmodelling/relatorio_cnj_final_1_inst.csv"  # texto_decisao
    )
    dados_2_inst = (
        "/home/ubuntu/topicmodelling/relatorio_cnj_final_2_inst_sp.csv"  # texto_decisao
    )
    ...
    def relatorio(path_dados, nome):
        ...
        df = pd.read_csv(path_dados, chunksize=1000, nrows=10000)
        topicos = models.ldamodel.LdaModel(
            [dicionario.doc2bow(text) for text in textos_0],
            num_topics=num_topics,
            id2word=dicionario,
            passes=npasses,
        )
