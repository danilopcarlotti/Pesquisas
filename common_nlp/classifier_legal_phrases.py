from closest_n_index import closest_n_index
...
class classifier_legal_phrases:
    ...
    def __init__(self, list_texts_control, treshold_distance=None):
        ...
        self.normal_texts = [
            " ".join(i) for i in self.txtN.normalize_texts(self.list_texts_control)
        ]
    ...
    def belongs_to_class(self, text):
        ...
        for x in self.X_control:
            ...
            if (
                mean_distance < smallest_dist
                and mean_distance <= self.treshold_distance / 2
            ):
...
def classify_sentences(text, path_classifiers="", nlp=None):
    ...
    dictionary_methods = {
        "fact": pickle.load(
            open(path_classifiers + "classifier_phrases_fact.pickle", "rb")
        ),
        "claim": pickle.load(
            open(path_classifiers + "classifier_phrases_claim.pickle", "rb")
        ),
        "decision": pickle.load(
            open(path_classifiers + "classifier_phrases_decision.pickle", "rb")
        ),
        "law": pickle.load(
            open(path_classifiers + "classifier_phrases_law.pickle", "rb")
        ),
    }
