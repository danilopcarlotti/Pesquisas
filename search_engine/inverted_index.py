        ...
        for f in files:
            ...
            for _, row in df.iterrows():
                if len(row[self.COLUMN_SOURCE]) > 10:
                    ...
                    normal_tokens = self.txtN.normalize_texts(
                    ...
                    self.insert_words_index(
                        counter_f, self.mydb[self.COLLECTION_INDEX], document_id
                    )
        ...
        for word in counter_f:
            ...
            if found_word:
                collection.update_one(
                    {"_id": word}, {"$push": {"documents": document_id}}
                )
            else:
                collection.insert_one({"_id": word, "documents": [document_id]})

    def update_index(self):
        for d in self.mydb[self.COLLECTION].find({}):
            normal_tokens = self.txtN.normalize_texts(
                d[self.COLUMN_SOURCE], one_text=True
            )
            counter_f = Counter(normal_tokens)
            self.insert_words_index(
                counter_f, self.mydb[self.COLLECTION_INDEX], d["_id"]
            )
