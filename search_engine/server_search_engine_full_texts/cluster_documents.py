...
class cluster_documents(main_class):
        ...
        for d in self.mydb[self.COLLECTION].find({}):
            if counter:
                ...
                collection.insert_one(
                    {"id_vector": ObjectId(d["_id"]), "collection": self.COLLECTION}
                )
                counter -= 1
            else:
                break
    ...
    def retrieve_data(self, text, report_name, prefix=""):
        ...
        for d in collection.find({}):
            if not counter_results:
                break
            data = self.mydb[self.COLLECTION].find_one(
                {"_id": ObjectId(d["id_vector"])}
            )
        ...
        try:
            ...
            for i in ind[0]:
                ...
                if data:
                    doc = Document()
                    doc.add_paragraph(
                        "Número do processo: " + data["numero_processo"] + "\n\n"
                    )
    ...
    def vectorizer_se(self, text):
        ...
        return [
            float(i)
            for i in list(self.vectorizer.transform([normal_text]).toarray()[0])
        ]
