...
def extracao_decisoes_cnj_csvs(lista_arquivos, path_final=""):
    ...
    for arq in lista_arquivos:
        ...
        try:
            try:
                df = pd.read_csv(
                    arq, usecols=["numero_processo", "tribunal", "texto_publicacao"]
                )
            except:
                df = pd.read_csv(
                    arq,
                    sep=";",
                    usecols=["numero_processo", "tribunal", "texto_publicacao"],
                )
            for _, row in df.iterrows():
                n_processo = consertar_ncnj(
                    str(row["numero_processo"]).replace(".", "").replace("-", "")
                )
                if n_processo in dicionario_acoes:
                    decisoes_interesse.append(
                        {
            ...
            if len(decisoes_interesse) > 200000:
                df_new = pd.DataFrame(
                    decisoes_interesse,
                    index=[i for i in range(len(decisoes_interesse))],
                )
                df_new.to_csv(
                    path_final
                    + "extração_ações_cnj_encontradas_texto_%s.csv"
                    % (
                        str(
                            counter,
                        )
    ...
    if len(decisoes_interesse):
        df_new = pd.DataFrame(
            decisoes_interesse, index=[i for i in range(len(decisoes_interesse))]
        )
        df_new.to_csv(
            path_final
            + "extração_ações_cnj_encontradas_texto_%s.csv"
...
def main():
    ...
    lista_arquivos = [
        i for i in rec.find_files("/home/deathstar/Dados/Diarios") if i[-3:] == "csv"
    ]
    print("Extraindo decisões")
    extracao_decisoes_cnj_csvs(
        lista_arquivos, path_final="/home/deathstar/Dados/Extracao_cnj_adocao/"
    )
