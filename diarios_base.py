...
class extracao_variaveis():
	...
	def variaveis_textos(self, dicionario, dados):
		...
		index = []
		index_counter = 1
		for tribunal, numero, data, texto in dados:
			try:
				est_ou_fed = 'estadual'
				if re.search(r'trf',str(tribunal)):
					est_ou_fed = 'federal'
				ano_processo = re.search(r'\.\d{4}\.',str(numero))
				if ano_processo:
					ano_processo = ano_processo.group(0)
				else:
					ano_processo = ''
				dicionario_df = {'numero':numero, 'ano': ano_processo, 'tribunal':tribunal, 'data':data,'estadual_federal':est_ou_fed,'justica_gratuita':self.parser.justica_gratuita(texto)}
...
def csv_base(filepath):
	...
	for index, row in df.iterrows():
		...
		# if re.search(r'alienação.{1,10}fiduciária',row['texto_publicacao'].lower(),re.DOTALL) and re.search(r'automóvel',row['texto_publicacao'].lower()):
			cursor.execute('INSERT INTO diarios_justica.publicacoes_diarias (tribunal, texto_publicacao, nome_arquivo, data, numero_processo) VALUES ("%s","%s","%s","%s","%s")' % (row['tribunal'],row['texto_publicacao'].replace('\n',' ').replace('"','').replace('\\',''),row['nome_arquivo'],row['data'],row['numero_processo']))
...
def main(path_diarios):
	...
	# rows = []
	# cursor = cursorConexao()
	# cursor.execute('SELECT tribunal, texto_publicacao, nome_arquivo, data, numero_processo from diarios_justica.publicacoes_diarias')
	...
	# data_frame.to_csv('/media/danilo/66F5773E19426C79/publicações_saúde.csv')
	...
	dados = []
	df = pd.read_csv(path_tutelas,error_bad_lines=False,chunksize=10000)
	for chunk in df:
		for index, row in chunk.iterrows():
			dados.append((row['tribunal'],row['numero_processo'],row['data'],row['texto_publicacao'].replace('"','')))
	df = ext.variaveis_textos(ext.var_bool, dados)
	df.to_csv('relatorio_booleano_cnj_15_jan_2019_tutelas.csv',quotechar='"', index= False)
	print('fim')
