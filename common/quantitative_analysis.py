...
from pesquisas.common_nlp.clustering_topics_regression_texts import texts_of_interest, topics_of_interest, cosine_dispersion, labels_to_csv
...
def main():
    PATH_FILES = '/home/deathstar/Documents/pesquisas_privado/previdenciario/extração_sentenças'
    rec = recursive_folders()
    paths_csv = [i for i in rec.find_files(PATH_FILES) if i[-3:] == 'csv' and not i.startswith('beta') and not re.search(r'todos_textos|labels_to_csv',i)]
    for csv_path in paths_csv:
        ...
        pca_plot(X, labels, 'Representation of clusters - PCA - 3 components',prefix=path_reports)
        # CSV OF LABELS
        print('Labels to csv')
