...
def graph_clusters(X, closest, labels, path_fig=''):
    ...
    for label, distance_m in [(label,mean_distance_cluster) for label, mean_distance_cluster in dicionario_distancias_medias.items()]:
        labels.append(label)
    ...
    distancias_centros_clusters = cosine_distances([v for _,v in dicionario_closest.items()],[v for _,v in dicionario_closest.items()])
    ...
    pickle.dump(distancias_centros_clusters,open(path_fig+'mutual distance of clusters.pickle','wb'))