...
class extracao_variaveis:
    ...
    def __init__(self):
        ...
        self.var_bool["Rename - remédio"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open(
                    "/media/danilo/66F5773E19426C79/CNJ_jud_saude/rename.txt", "r"
                )
            ]
        )
        self.var_bool["Medicamentos atenção básica"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open(
                    "/media/danilo/66F5773E19426C79/CNJ_jud_saude/medicamentos_atencao_basica.txt",
                    "r",
                )
            ]
        )
        self.var_bool["Medicamentos componente especializado"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open(
                    "/media/danilo/66F5773E19426C79/CNJ_jud_saude/medicamentos_componente_especializado.txt",
                    "r",
                )
            ]
        )
        self.var_bool["Medicamentos estratégicos"] = "|".join(
            [
                i.replace(" + ", ".{1,20}").strip()
                for i in open(
                    "/media/danilo/66F5773E19426C79/CNJ_jud_saude/medicamentos_estrategicos.txt",
                    "r",
                )
            ]
        )
        ...
        for (
            ...
            try:
                ...
                dicionario_df = {
                    ...
                    "origem": origem,
                    "estadual_federal": est_ou_fed,
                    "resultado": self.parser.indicaResultado(texto, "Recurso"),
                    "justica_gratuita": self.parser.justica_gratuita(texto),
                }
...
def main():
    ...
    df_saude = pd.read_csv(
        "/media/danilo/66F5773E19426C79/CNJ_jud_saude/relatorio_cnj_final_2_inst_sp.csv",
        encoding="utf8",
    )
    ...
    for row in df_saude.itertuples():
        dados.append(
            (
                getattr(row, "numero"),
                getattr(row, "texto_decisao"),
                getattr(row, "tribunal"),
                getattr(row, "data_decisao"),
                getattr(row, "polo_ativo"),
                getattr(row, "polo_passivo"),
                getattr(row, "origem"),
            )
        )
    df = ext.variaveis_textos(ext.var_bool, dados)
    df["data_decisao"] = pd.to_datetime(
        df["data_decisao"], format="%d/%m/%Y", errors="coerce"
    )
    df.to_csv(
        "relatorio_booleano_cnj_16_jan_2019_2_inst_sp.csv", quotechar='"', index=False
    )
